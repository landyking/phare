    ...
    <div class="layui-card-body">
        ...
        <script type="text/html" id="tableToolbar">
            <div class="layui-btn-container">
                <button class="layui-btn layui-btn-sm layui-btn-normal"
                        lay-data="{url:'_admin/tableColumn/addTableColumn.jsp',params:{tableId:hy.getURLParam('id'),tableCode:hy.getURLParam('code')},width:600,height:600}"
                        lay-event="dialog">新增
                </button>
            </div>
        </script>
        <script type="text/html" id="tableRowMenu">
            <a class="layui-btn layui-btn-xs" lay-event="dialog"
               lay-data="{url:'_admin/tableColumn/updateTableColumn.jsp',params:{id:'?'}}">编辑</a>
            <a class="layui-btn layui-btn-danger layui-btn-xs"
               lay-data="{url:'admin/tableColumn/deleteTableColumn',params:{id:'?'}}" lay-event="ajax">删除</a>
        </script>
    </div>
</div>
</body>
...
<script>
    layui.use(['singleTableList', 'layer', 'hyUtil'], function () {
        var tableId = hy.getURLParam("id");
        layui.singleTableList.render({
            layFilter: 'dataTable',
            tableConfig: {
                limit: 90,
                url: 'admin/tableColumn/listTableColumn?tableId=' + tableId,
                cols: [[
                    {type: 'numbers', fixed: true},
                    //{checkbox: true, fixed: true},
                    {field: 'id', title: 'ID', hide: true},
                    {field: 'code', title: '编码', width: 150},
                    {field: 'tableCode', title: '所属表', hide:true},
                    {field: 'name', title: '名称'},
                    {field: 'description', title: '描述', hide: true},
                    {field: 'nullableFlag', title: '可空'},
                    {field: 'pkFlag', title: '是否主键'},
                    {field: 'fkId', title: '外键引用id', hide: true},
                    {field: 'defVal', title: '默认值', hide: true},
                    {field: 'dataType', title: '数据类型', hide: true},
                    {field: 'dbType', title: '数据库类型'},
                    {field: 'typeLength', title: '类型长度', hide: true},
                    {field: 'typePrecision', title: '类型精度', hide: true},
                    {field: 'constantId', title: '常量引用id', hide: true},
