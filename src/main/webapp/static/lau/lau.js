layui.define(['layer', 'laytpl', 'element'], function (exports) {
    ...
    var Layout = function () {
        ...
        //加载顶级菜单
        var topHref = $.trim(TOP.data('href'));
        if (topHref) {
            $.ajax({
                url: topHref,
                dataType: 'json',
                success: function (rst, textStatus, jqXHR) {
                    ...
                    $.each(data, function (idx, one) {
                        var ele = $('<li class="layui-nav-item" lay-unselect><a href="javascript:;">' + one.title + '</a></li>');
                        ele.find('a').click(function () {
                            TOP.find(".layui-this").removeClass('layui-this');
                            ele.addClass("layui-this");
                            THIS.sideMenuChange(idx);
                        });
                        TOP.append(ele);
                        if (one.list) {
                            sideData.push(one.list)
                        }
                    });
                    element.render('nav');
                    THIS.sideMenuLoad(sideData);
                },
        ...
        function traceMenu() {
            ...
            SIDE.find('.layui-this').removeClass('layui-this');
