...
public abstract class BaseController extends AbstractController {
    ...
    protected Properties getAppProps() {
        return getApplicationContext().getBean("appProps", Properties.class);
    }
    ...
    protected MyUser getMyUser(int userType) {
        Subject subject = SecurityUtils.getSubject();
        if (subject.isAuthenticated()) {
            if (subject.getPrincipal() instanceof MyUser) {
                MyUser user = (MyUser) subject.getPrincipal();
                if (user.getUserMode() == userType) {
                    return user;
                }
            }
        }
